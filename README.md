BERT head 방식을 이용한 렉사일 지수 예측기

렉사일 지수란?

영어 읽기 지수로, 학생의 읽기 능력과 글의 독해 난이도를 같은 척도(scale)로 측정할 수 있다는 특징을 가짐

렉사일 지수는 개별 학생들에게 맞춤형으로 읽기 자료를 추천해줄 수 있다는 점에서 교육적인 의미가 큼

그러나 렉사일 지수를 사용하는 데에는 두 가지의 아주 큰 한계점이 존재한다.

첫째, 렉사일 지수를 측정하는 방법이 대중에게 공개가 되어 있지 않다. 해당 지수를 개발한 MetaMetrics 회사만이 측정하는 기술을 보유하고 있다.

둘째, 만약 교수자가 특정 자료의 렉사일 지수를 알고자 한다면, MetaMetrics에게 돈을 지불해야 한다.

교수자가 돈을 지불할 여력이 없다면?

더욱 질 좋은 수업을 제공할 수 있었음에도 불구하고 단지 금전적인 문제로 인해 좋은 수업을 제공하지 못하게 된다.


필자는 이러한 상황에서 교수자들이 렉사일 지수를 무료로 예측할 수 있도록 도움을 주고자 해당 딥 러닝 모델을 만들게 되었다. 
